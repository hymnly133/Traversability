...
def verify_payload() -> None:
    ...
    require("referenceTerrain" not in payload, "raw simulation elevation grid leaked into the interactive payload")
    ...
    temporary = plan["temporaryElevationMap"]
    require(temporary["source"] == "local_point_cloud_window", "temporary elevation map was not derived from the local point cloud window")
    require(temporary["rows"] > 0 and temporary["cols"] > 0, "temporary elevation map is empty")
    require(temporary["rows"] * temporary["cols"] == summary["local_cells"], "temporary elevation map does not match the local map window")
...
def verify_frontend_source() -> None:
    ...
    require("data-layer=\"tempElevation\"" in INDEX_HTML and "临时高程图" in INDEX_HTML, "frontend lacks temporary elevation map toggle")
    require("function drawTemporaryElevationMap" in INDEX_HTML and "temporaryElevationMap" in INDEX_HTML, "frontend does not render the point-cloud-derived temporary elevation map")
    require("referenceTerrain" not in INDEX_HTML and "drawReferenceTerrain" not in INDEX_HTML, "frontend still references the raw elevation grid as a display layer")
...
def verify_summary(summary: dict) -> None:
    ...
    require(summary["planning_map_source"] == "point_cloud_derived", "interactive planning used a non-point-cloud map source")
